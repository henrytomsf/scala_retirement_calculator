...
class EquityDataSpec extends AnyFlatSpec {
    "EquityData.fromResource" should "load market data from a tsv file" in {
        val fileName = "sp500_2017.tsv"
        val data = EquityData.fromResource(fileName)
        data === (
            Vector(
    ...
    "EquityData.monthlyDividend" should "return a monthly dividend" in {
        EquityData("2016.09", 2157.69, 45.03).monthlyDividend === (45.03/12)
    }
}